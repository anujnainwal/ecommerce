...
const register_address_validation = Joi.object({
  ...
  country_code: Joi.number().positive().required(),
  ...
  addressType: Joi.string().required().valid("home", "work").lowercase(),
});

const update_address_validation = Joi.object({
  name: Joi.string()
    .min(3)
    .max(50)
    .regex(/^[a-zA-Z\s]+$/)
  ...
  country_code: Joi.number().positive().optional(),
  phoneNumber: Joi.string()
    .pattern(/^\d{10}$/)
    .optional()
    .messages({
  ...
  addressType: Joi.string().optional().valid("home", "work").lowercase(),
});

export { register_address_validation, update_address_validation };
